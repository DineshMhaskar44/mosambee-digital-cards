                  ...
                  <tr key={emp.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <Link href={`/card/${emp.employee_id}`} target="_blank"
                          className="p-1.5 rounded-lg text-gray-400 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors" title="View Card">
                          <ExternalLink className="w-4 h-4" />
                        </Link>
                        <Link href={`/admin/employees/${emp.id}/edit`}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors" title="Edit">
                          <Edit className="w-4 h-4" />
                        </Link>
                        <button onClick={() => handleDownloadQR(emp.id)}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors" title="Download QR">
                          <Download className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleRegenQR(emp.id)} disabled={regening === emp.id}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors disabled:opacity-50" title="Regenerate QR">
                          <RefreshCw className={`w-4 h-4 ${regening === emp.id ? "animate-spin" : ""}`} />
                        </button>
                        <button onClick={() => handleDelete(emp.id, emp.full_name)} disabled={deleting === emp.id}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors disabled:opacity-50" title="Delete">
