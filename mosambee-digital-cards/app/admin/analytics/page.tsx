...
export default async function AnalyticsPage() {
  ...
  const [
    ...
    supabase.from("scan_logs").select("id", { count: "exact", head: true }).gte("scanned_at", new Date().toISOString().split("T")[0]),
    supabase.from("scan_logs").select("id", { count: "exact", head: true }).gte("scanned_at", new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString()),
